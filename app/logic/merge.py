...
def merge_facts(project: Project, facts: ExtractedFacts, page: Page, verified_sources: set[str] = frozenset({"maharera"})) -> Project:
    ...
    if facts.configurations:
        project.configurations.append(FieldValue(value=sorted(set(facts.configurations)), prov=prov))
    if facts.carpet_min_sqft and facts.carpet_max_sqft:
        project.carpet_sqft.append(FieldValue(value=CarpetRange(min_sqft=facts.carpet_min_sqft, max_sqft=facts.carpet_max_sqft), prov=prov))
    if facts.rate_min_psf or facts.rate_max_psf:
        ...
        project.rate_psf.append(FieldValue(value=RateValue(min_psf=lo, max_psf=hi, basis=facts.rate_basis or "undisclosed"), prov=prov))
    poss = parse_month(facts.possession)
    if poss:
        project.possession.append(FieldValue(value=poss, prov=prov))
    if any(v is not None for v in (facts.building_type, facts.towers, facts.floors_min, facts.land_acres, facts.total_units)):
        ...
        project.structure.append(FieldValue(value=Structure(
    ...
    if facts.rera_numbers:
        ...
        already = {(fv.prov.source, tuple(ph.number for ph in fv.value)) for fv in project.rera_phases}
        if nums and (page.source, tuple(dict.fromkeys(nums))) not in already:
            ...
            project.rera_phases.append(FieldValue(value=phases, prov=prov))
    if facts.amenities:
        ...
        project.amenities.append(FieldValue(value=list(seen.values()), prov=prov))
    launched = parse_month(facts.launched)
    if launched or facts.extensions_filed is not None or facts.construction_stage:
        project.timeline.append(FieldValue(value=Timeline(
...
def consolidate_rera(project: Project) -> Project:
    ...
    verified = {ph.number for fv in project.rera_phases if fv.prov.source == "maharera" for ph in fv.value}
    for fv in project.rera_phases:
        for ph in fv.value:
